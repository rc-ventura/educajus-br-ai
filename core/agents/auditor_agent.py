...
def execute(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (
        blocks.get("error_pii")
        or blocks.get("error_scope")
        or blocks.get("error_insufficient_sources")
    ):
        return state
    ...
    if alignment_score < 0.7:
        ...
        validation_results["issues"].append(
            f"Low alignment score: {alignment_score:.2f}"
        )

    # 2. Validate citations are in sources
    citations_valid = _validate_citations(
        content_blocks.get("legal_basis", []), sources
    )
    ...
    if not is_educational:
        validation_results["is_valid"] = False
        validation_results["issues"].append(
            "Content contains personalized legal advice"
        )
    ...
    logger.info(
        f"Auditor validation: valid={validation_results['is_valid']}, issues={len(validation_results['issues'])}"
    )
...
Responda apenas: educational ou advice"""
        ...
        classification = (
            (response_llm.choices[0].message.content or "educational").strip().lower()
        )
